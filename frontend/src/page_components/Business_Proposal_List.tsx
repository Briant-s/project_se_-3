...
function BusinessProposalList() {
  ...
  const toProposalDraft = useCallback(
    (proposal: BusinessProposal): ProposalDraft => {
  ...
  useEffect(() => {
    const fetchProposals = async () => {
      ...
      try {
        ...
        const sorted = result.map(toProposalDraft).sort((a, b) => {
  ...
  const confirmDeleteProposal = (id: string) => {
    openConfirmModal({
      ...
      children: (
        <Text size="sm">
          Are you sure you want to delete this proposal draft? This action
          cannot be undone.
        </Text>
      ),
            ...
            {proposals.length === 0 && (
              <Card radius="md" withBorder p="xl">
                <Stack align="center" gap="sm" py="xl">
                  ...
                  <Text size="sm" c="dimmed">
                    Create your first business proposal now.
                  </Text>
                  <Button
                    variant="light"
                    onClick={() => navigate("/business-proposal")}
                  >
                    Create Now
                  </Button>
                </Stack>
                ...
                >
                  ...
                  <Group gap="xs" mt="xs">
                    <Button
                      ...
                      onClick={() =>
                        proposal.id != null &&
                        navigate(`/ai-business-proposal/result/${proposal.id}`)
                      }
                    ...
                    <Menu shadow="md" width={160} position="bottom-end">
                      ...
                      <Menu.Dropdown>
                        <Menu.Item
                          onClick={() =>
                            proposal.id &&
                            navigate(`/business-proposal/edit/${proposal.id}`)
                          }
                          // disabled={!proposal.id}
                        >
                          Edit
                        ...
                        <Menu.Item
                          ...
                          onClick={() =>
                            proposal.id != null &&
                            confirmDeleteProposal(String(proposal.id))
                          }
                          // disabled={!proposal.id}
                        >
                          Delete
